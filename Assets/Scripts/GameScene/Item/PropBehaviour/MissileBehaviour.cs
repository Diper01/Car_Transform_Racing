...
public class MissileBehaviour : BehaviourBase {
	...
	public void Launch(float pathLen, float offset, CarMove targetCarMove,float mOffset)
	{
		...
		ahead = RoadPathManager.Instance.GetPointByLen (pathLen + 1f, offset);
        direction = ahead - selfTrans.position;
		selfTrans.forward = direction;
	...
	void MoveToNextPoint()
	{
		...
		ahead= RoadPathManager.Instance.GetPointByLen(moveLen + 1f, xOffset);
        direction = ahead - selfTrans.position;
		selfTrans.forward = Vector3.Lerp(selfTrans.forward, direction, 10*Time.deltaTime);
