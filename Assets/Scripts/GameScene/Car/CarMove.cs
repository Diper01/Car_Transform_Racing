...
public class CarMove : MonoBehaviour {
	...
	void Update()
	{
		if(applyStartPos)//Почему не в старте?
		{
		    Vector3 pos=  RoadPathManager.Instance.GetPointByLen(startPosLen,xOffset);
			Vector3 ahead=  RoadPathManager.Instance.GetPointByLen(startPosLen+2f,xOffset);
			transform.position= pos;
	...
	void FixedUpdate ()
    {
		if(gameObject==null || !gameObject.activeInHierarchy||GameData.Instance.IsPause)//сделать нормально || carType == CarType.NormalCar
        {
			return;
		}
	...
	void MoveToNextPoint()
	{
        ...
        //Это надо упростить. Нет смысла каждый кадр проверять всю дорогу.
