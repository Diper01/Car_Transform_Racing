...
public class CarManager : MonoBehaviour {
    ...
    void UpdateOpponentCarSpeedChange()
	{
		...
		playerMove2 = carMoveList[0];
		for(int i=1;i<carMoveList.Count;++i)
		{
			oppCar2 = carMoveList[i];
			if(oppCar2.moveLen>playerMove2.moveLen+aheadSpeedDownDis )
			{
				oppCar2.maxSpeed=playerMove2.speed-speedUpChangeValue;
			}
			else if(oppCar2.moveLen<playerMove2.moveLen-aheadSpeedDownDis )
			{
				oppCar2.maxSpeed=playerMove2.speed+speedDownChangeValue;
			}else if( !oppCar2.propCon.isSpeedUp  && !oppCar2.propCon.isMushroom 
			         && !oppCar2.propCon.isShapeShift && !oppCar2.propCon.isSpeedAdd )
			{

				oppCar2.maxSpeed= oppCar2.origMaxSpeed;
			}
		}
	}
	#endregion
    ...
    private void UpdateOpponentTips()
	{
		...
		playerMove = carMoveList[0];
		for(int i=1;i<carMoveList.Count;++i)
		{
			oppCar = carMoveList[i];
		...
		{
			closeCar=carMoveList[closestIndex];
