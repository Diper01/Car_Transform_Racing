...
public class PropControl : MonoBehaviour {
    ...
    List<CarMove> carSpeedUpMoveList = new List<CarMove>();
    public void StartSpeedUp(float lastTime)
	{
        ...
        //眩晕其他车手1秒
        carSpeedUpMoveList.Clear();
        carSpeedUpMoveList.AddRange (CarManager.Instance.carMoveList);
        carSpeedUpMoveList.Remove (carMove);
		for (int i = 0; i < carSpeedUpMoveList.Count; ++i) {
            carSpeedUpMoveList[i].CarLighting (GameData.Instance.SpeedUpLightingVertigoTime);
		}
    ...
    List<CarMove> carMoveListIEStartMissile;

    IEnumerator IEStartMissile(int missileCount)
	{
        carMoveListIEStartMissile = CarManager.Instance.GetPreCarList (carMove, missileCount);
		...
		for (int i = 0; i < missileCount; ++i) {
			...
			if (carMoveListIEStartMissile.Count > i)
				targetCarMove = carMoveListIEStartMissile[i];
			else
				targetCarMove = null;
    ...
    FlyBmobBehaviour flyBmobSript;
    Transform flyBmobTran;
    IEnumerator IEStartFlyBomb(int bombCount)
	{
        for (int i = 0; i < 5; ++i) {
			for (int j = 1; j < bombCount; ++j) {
                flyBmobTran = itemPool.Spawn ("FlyBombBehaviour");
                flyBmobSript = flyBmobTran.GetComponent<FlyBmobBehaviour> ();
    ...
    List<CarMove> carMoveListIEStartLighting = new List<CarMove>();
    private IEnumerator IEStartLighting()
	{
        ...
        carMoveListIEStartLighting.Clear();

        carMoveListIEStartLighting.AddRange (CarManager.Instance.carMoveList);
        carMoveListIEStartLighting.Remove (carMove);
		for (int i = 0; i < carMoveListIEStartLighting.Count; ++i) {
            carMoveListIEStartLighting[i].propCon.LightingHit ();
		}
		if (isPlayer) {
