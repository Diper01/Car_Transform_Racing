...
public class DropItemManage : MonoBehaviour {
	...
	Vector3 GetUIPos(Vector3 worldPos)
	{
		...
		screenPos.z = 0;
		Vector3 uiPos = uiCamera.ScreenToWorldPoint (screenPos);//Камера выключена, переделать это под UI. Когда игрок сталкивается с монетками, происходит анимация и монетки появляются на экране.
		uiPos.z=0f;
		return uiPos;
	}
