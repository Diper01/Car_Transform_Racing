...
public class MissionBoard : MonoBehaviour {
	...
	private void InitMission()
	{
		...
		for (int i = 0; i < missionIdArr.Length; i ++) {
			...
			mission.countText = mission.showTrans.Find("Count_txt").GetComponent<Text>();
			mission.desText = mission.showTrans.Find("Desc_txt").GetComponent<Text>();
    ...
    MissionStruct missionCheck;

    void UpdateLimitTimeCheck()
	{
        missionCheck = GetMissionByType (MissionType.LimitTime);
		if (missionCheck.isComplete == false) {
			float leftSec = CarManager.Instance.totalUseTime - CarManager.Instance.playerUseTime;
            missionCheck.curNum = leftSec;
			SetMissionItemData(missionCheck, false);
		}
	}
    Transform tranMission;

    void InitMissionItem(MissionStruct mission)
	{
		tranMission = mission.showTrans;
        ...
        //mission.desText.text = Localisation.GetString(MissionData.Instance.GetGameDes(missionID));

        // New mission name
        string missionText = MissionData.Instance.GetDesc(missionID).Replace("N", iMissionNum.ToString());
        mission.desText.text = Localisation.GetString(missionText);
    }
    ...
    void SetMissionItemData(MissionStruct mission,bool isWithComplete=true)
	{
		...
		if (curNum >=maxNum && mission.isComplete==false) {
			...
			tranMission.Find("Complete_icon").gameObject.SetActive(true);
