...
public class ConvertItem : MonoBehaviour {
    ...
    public tk2dUIItem goConvertButtonUIItem;
	...
	public void SetProgress(float percent)
	{
		if (percent >= 1) {
            ...
            if (gameObject.activeInHierarchy)
                PublicSceneObject.Instance.AddButtonToSelectList(goConvertButtonUIItem);
			TipText.gameObject.SetActive (false);
		} else {
			convertFlag = false;
			goConvertButton.SetActive (false);
                PublicSceneObject.Instance.RemoveButtonFromSelectList(goConvertButtonUIItem);
