...
public class SmashEggControllor : UIBoxBase {
    ...
    public tk2dUIItem hitButtonGOUIItem;

    //private void Awake()
    //{
    //    hitButtonGOUIItem = hitButtonGO.GetComponent<tk2dUIItem>();
    //}
	...
	void SetData()
	{
		...
		//titleText.text = Localisation.GetString(SmashEggData.Instance.GetTitle ());
		desText.text = Localisation.GetString(SmashEggData.Instance.GetDes ());
	...
	private void  SetCanSmash(bool isCanSmash)
	{
        ...
        if (!isCanSmash)
            PublicSceneObject.Instance.RemoveButtonFromSelectList(hitButtonGOUIItem);
        else
            if (gameObject.activeInHierarchy)
            PublicSceneObject.Instance.AddButtonToSelectList(hitButtonGOUIItem);
	...
	private void InitTimeRecover()
	{
		...
		if (leftUseTime >= TotalTime) {
			//have been used
			SetCanSmash(false);
			timeText.text= Localisation.GetString("Come back tomorrow");
			return;
		}
		...
		if (deltaSecond >= intervalSecond && (leftUseTime < TotalTime))
		{
			//can smash egg
			SetCanSmash(true);
			timeText.text = Localisation.GetString("Smash Egg");
	...
	private void UpdateTimeRecorver()
	{
		...
		if (leftSecond <= 0 && (leftUseTime < TotalTime))
		{
			CancelInvoke("UpdateTimeRecorver");
			SetCanSmash(true);
			timeText.text = Localisation.GetString("Smash Egg");
