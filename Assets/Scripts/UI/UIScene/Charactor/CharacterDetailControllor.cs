...
public class CharacterDetailControllor : UIBoxBase {
    ...
    //private void Awake()
    //{
    //    goZaohuanButtonUIItem = goZaohuanButton.GetComponent<tk2dUIItem>();
    //}
    #region 重写父类方法
...
//			}
	...
	public override void Show ()
	{
		...
		if (GlobalConst.IsSignIn && PlayerData.Instance.GetUIGuideState (UIGuideType.CharaterDetailGuide) && PlayerData.Instance.GetCurrentChallengeLevel () == 3) {
            ...
            {
                //Debug.Log("CharaterDetailGuide Level 3");
...
//		if(modelLevel <= AutoGiftChecker.MaxModelLevelOpenOneKeyToFull)
   ...
   //         PublicSceneObject.Instance.RemoveButtonToSelectList(goOneKeyToFullButtonUIItem);
			...
			if(upgradeType.CompareTo("Coin") == 0)
			{
				...
				CoinTypeBtnText.text = Localisation.GetString("Buy");
			}
			else
			{
				...
				JewelTypeBtnText.text = Localisation.GetString("Buy");
			}
		}
		else
		{
    ...
    //                PublicSceneObject.Instance.AddButtonToSelectList(goOneKeyToFullButtonUIItem);
    ...
    //                goZaohuanButton.transform.localPosition = new Vector3(0, -185 + UIResAdjust.AdjustY(-1), 0);
				...
				if(upgradeType.CompareTo("Coin") == 0)
				{
					...
					CoinTypeCostTypeImage.SetSprite (upgradeType.ToLower() + "_icon");
					CoinTypeCostCountText.text = upgradeCount.ToString();
					CoinTypeBtnText.text = Localisation.GetString("Refit");
				}
				else
				{
					CoinTypeGO.SetActive(false);
					...
					JewelTypeCostTypeImage.SetSprite (upgradeType.ToLower() + "_icon");
					JewelTypeCostCountText.text = upgradeCount.ToString();
					JewelTypeBtnText.text = Localisation.GetString("Refit");
				}
			}
			else
			{
	...
	public void SetModelInfo(int Id)
	{
		...
		if(modelLevel == 0)
			textTitle.text = Localisation.GetString(ModelData.Instance.GetName(Id));
		else
			textTitle.text = Localisation.GetString(ModelData.Instance.GetName(Id)) + " " + Localisation.GetString("Lvl") + " " +modelLevel;
		...
		textBuff1Name.text = Localisation.GetString(ModelData.Instance.GetLevelSkill (Id));
		textBuff1Info.text = Localisation.GetString(ModelData.Instance.GetLevelSkillDesc (Id));

		if (modelLevel < skillLevel) {
			enableDesc1Text.text = Localisation.GetString("Level：") + skillLevel; // Level activation
        } else {
			enableDesc1Text.text = Localisation.GetString("Level：") + skillLevel;
