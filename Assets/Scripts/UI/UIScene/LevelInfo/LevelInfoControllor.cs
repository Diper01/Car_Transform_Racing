...
public class LevelInfoControllor : UIBoxBase {
    ...
    public tk2dUIItem goStartButtonUIItem;
	...
	public void InitData(int levelId)
	{
		...
		LevelText.text = Localisation.GetString("Race") + " "+levelId.ToString () + "";
...
//			modelAnimArr[i].Idle();
    ...
    public override void Show()
    {
		...
		else if (guide&&PlayerData.Instance.GetUIGuideState (UIGuideType.LevelInfoGuide) && PlayerData.Instance.GetCurrentChallengeLevel () == 4)
        {
            guide = false;
            //Debug.Log("LevelInfoGuide Level 4");
            UIGuideControllor.Instance.Show (UIGuideType.LevelInfoGuide);
			UIGuideControllor.Instance.ShowBubbleTipByID(1);
			TranslucentUIMaskManager.Instance.SetLayer (12);
			//ActorCameraController2.Instance.SetCameraDepth (4);//Было закоментировано
		}
	...
	private void SetButtonData()
	{
		...
		if (modelLevel == 0) {
            ...
            PublicSceneObject.Instance.RemoveButtonFromSelectList(goStartButtonUIItem);
            ...
            if (gameObject.activeInHierarchy)
                PublicSceneObject.Instance.AddButtonToSelectList(goStartButtonUIItem);
        }
	}
