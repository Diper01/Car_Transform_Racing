...
public class DailyMissionItem : MonoBehaviour {
    ...
    [SerializeField]
    private tk2dUIItem getBtnGoUIItem;
    ...
    //private void Awake()
    //{
    //    getBtnGoUIItem = getBtnGo.GetComponent<tk2dUIItem>();
    //}
    ...
    public void SetData(int missionId)
	{
		...
		descText.text = Localisation.GetString(itemData.missionDes);
		...
		if (itemData.missionCurCount < itemData.missionTarget) {
            ...
            PublicSceneObject.Instance.RemoveButtonFromSelectList(getBtnGoUIItem);
            ...
            if (gameObject.activeInHierarchy)
                PublicSceneObject.Instance.AddButtonToSelectList(getBtnGoUIItem);
	...
	IEnumerator IEChangeAnimate()
	{
		...
		transform.DOLocalMoveX(-13,0.2f);
	}
}
