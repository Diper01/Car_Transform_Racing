...
public class MainInterfaceControllor : UIBoxBase {
    ...
    public tk2dUIItem goZaohuanButtonUIItem;

    //private void Awake()
    //{
    //    goZaohuanButtonUIIteml = goZaohuanButton.GetComponent<tk2dUIItem>();
    //}
    void OnEnable()
	{
        StartCoroutine("CheckAutoGiftIE");
        //foreach(EasyFontTextMesh item in GetComponentsInChildren<EasyFontTextMesh>())
        //{
	...
	{
        //Debug.Log(UpgradeCountText);
        //Debug.Log(UpgradeCountText.text);
	...
	public override void Show ()
	{
		...
		if (GlobalConst.StartGameGuideEnabled) {
            {
                ...
                //UIGuideControllor.Instance.ShowBubbleTipByID(16);
                HideFingerGuide();
            }
		} else if (GlobalConst.IsSignIn && !LevelInfoControllor.show&&PlayerData.Instance.GetCurrentChallengeLevel () == 3 && PlayerData.Instance.GetUIGuideState (UIGuideType.CharaterDetailGuide)) {
		...
		} else if (GlobalConst.IsSignIn && GlobalConst.LeftRoleGuideEnabled &&!LevelInfoControllor.show)//Возможно потому что перезашел в игру
        {
			UIGuideControllor.Instance.Show (UIGuideType.LeftRoleGuide);
            //Debug.Log("LeftRoleGuide Level 4");
	...
	public void HideFingerGuide()
	{
		...
		FingerGuideTran = transform.Find("Buttons").Find("LevelGameButton").Find("FingerGuide");
	...
	private void SetButtonData()
	{
		...
		DescText.text = Localisation.GetString(tempDesc);
//		DescText.text = temp[0] + "\n" + temp[1];// ModelData.Instance.GetDesc (modelType);
		...
		if(modelLevel == 0)
		{
            NameText.text = Localisation.GetString(ModelData.Instance.GetName(modelAllStateIds[modelIndex]));
			upgradeType = ModelData.Instance.GetZhaohuanCostType(modelAllStateIds[modelIndex]);
			upgradeCount = ModelData.Instance.GetZhaohuanCost(modelAllStateIds[modelIndex]);//error

			goMaxLevelButton.SetActive(false);
			goZaohuanButton.SetActive(true);

            PublicSceneObject.Instance.AddButtonToSelectList(goZaohuanButtonUIItem);
		...
		{
			NameText.text = Localisation.GetString(ModelData.Instance.GetName (modelAllStateIds[modelIndex])) + " " + Localisation.GetString("Lvl") + modelLevel;
			...
			else
			{
                ...
                PublicSceneObject.Instance.RemoveButtonFromSelectList(goZaohuanButtonUIItem);
            }
		}
    ...
    //{
    //    goCharactersButton.SetActive(false);
    //    foreach(EasyFontTextMesh item in texts)
    //    {
	...
	public void LevelGameOnClick()
	{
        ...
        //新角色引导流程
        if (guide && GlobalConst.LeftRoleGuideEnabled)
        {
            guide = false;
